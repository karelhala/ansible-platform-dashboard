...
describe('<AnalyticsCard />', () => {
  ...
  it('should render correctly', async ()=> {
    ...
    let wrapper;
    await act(async () => {
      wrapper = shallow(<ComponentWrapper store={ store }><AnalyticsCard { ...initialProps } /></ComponentWrapper>);
    });
    wrapper.update();
  ...
  it('should render correctly in loading state', () => {
    const store = mockStore(initialState);
    let wrapper;
    shallow(<ComponentWrapper store={ store }><AnalyticsCard { ...initialProps } isLoading={ true } /></ComponentWrapper>);
    expect(shallowToJson(wrapper)).toMatchSnapshot();
  });
});
